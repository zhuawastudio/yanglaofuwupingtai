		...
		<div v-if="showFlag">
			<el-form :inline="true" :model="searchForm" class="form-content">
				<el-row :gutter="20" class="slt"
					...
					<el-form-item :label="contents.inputTitle == 1 ? '医生姓名' : ''">
						<el-input prefix-icon="el-icon-search" v-model="searchForm.yishengName" placeholder="医生姓名"
							clearable></el-input>
					</el-form-item>


					<el-form-item :label="contents.inputTitle == 1 ? '老人姓名' : ''">
						<el-input prefix-icon="el-icon-search" v-model="searchForm.yonghuName" placeholder="老人姓名"
							clearable></el-input>
					</el-form-item>
				...
				<el-row class="ad"
					...
					<el-form-item>
						<el-button v-if="isAuth('zhongyizhenduan','新增')" type="success" icon="el-icon-plus"
							@click="addOrUpdateHandler()">新增</el-button>
						&nbsp;
						<el-button v-if="isAuth('zhongyizhenduan','删除')" :disabled="dataListSelections.length <= 0"
							type="danger" icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
						&nbsp;
						<el-button v-if="isAuth('zhongyizhenduan','报表')" type="success" icon="el-icon-pie-chart"
							@click="chartDialog()">报表</el-button>
						&nbsp;
						<a style="text-decoration:none" class="el-button el-button--success"
							v-if="isAuth('zhongyizhenduan','导入导出')" icon="el-icon-download"
							href="http://localhost:8080/yanglaofuwupingtai/upload/zhongyizhenduanMuBan.xls">批量导入中医诊断数据模板</a>
						&nbsp;
						<el-upload v-if="isAuth('zhongyizhenduan','导入导出')" style="display: inline-block"
							action="yanglaofuwupingtai/file/upload" :on-success="zhongyizhenduanUploadSuccess"
							:on-error="zhongyizhenduanUploadError" :show-file-list=false>
							<el-button v-if="isAuth('zhongyizhenduan','导入导出')" type="success" icon="el-icon-upload2">
								批量导入中医诊断数据</el-button>
						</el-upload>
						&nbsp;
						<!-- 导出excel -->
						<download-excel v-if="isAuth('zhongyizhenduan','导入导出')" style="display: inline-block"
							class="export-excel-wrapper" :data="dataList" :fields="json_fields"
							name="zhongyizhenduan.xls">
							<!-- 导出excel -->
			...
			<div class="table-content">
				<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					v-if="isAuth('zhongyizhenduan','查看')" :data="dataList" v-loading="dataListLoading"
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="yishengUuidNumber" header-align="center" label="医生工号">
						<template slot-scope="scope">
							{{scope.row.yishengUuidNumber}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yishengName"
						header-align="center" label="医生姓名">
						<template slot-scope="scope">
							{{scope.row.yishengName}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yishengPhone"
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yonghuName"
						header-align="center" label="老人姓名">
						<template slot-scope="scope">
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="zhongyizhenduanText" header-align="center" label="症状">
						<template slot-scope="scope">
							<span
								v-if="scope.row.zhongyizhenduanText != null &&scope.row.zhongyizhenduanText.length>10">
								{{scope.row.zhongyizhenduanText.slice(0,10)}}...
							</span>
							<span v-else>
								{{scope.row.zhongyizhenduanText}}
							</span>
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="zhongyizhenduanHuifuText" header-align="center" label="回复">
						<template slot-scope="scope">
							<span
								v-if="scope.row.zhongyizhenduanHuifuText != null &&scope.row.zhongyizhenduanHuifuText.length>10">
								{{scope.row.zhongyizhenduanHuifuText.slice(0,10)}}...
							</span>
							<span v-else>
								{{scope.row.zhongyizhenduanHuifuText}}
							</span>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
						<template slot-scope="scope">
							<el-button v-if="isAuth('zhongyizhenduan','查看')" type="success" icon="el-icon-tickets"
								size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button v-if="isAuth('zhongyizhenduan','修改')" type="primary" icon="el-icon-edit"
								size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
							<el-button v-if="isAuth('zhongyizhenduan','删除')" type="danger" icon="el-icon-delete"
		...
		data() {
			return {
				...
				form: {
					...
					yonghuId: null,
					yishengId: null,
					zhongyizhenduanText: null,
					zhongyizhenduanHuifuText: null,
					createTime: null,
				},
				dataList: [],
				pageIndex: 1,
				...
				json_fields: {
					...
					'头像': 'yonghuPhoto',
					'联系方式': 'yonghuPhone',
					'老人身份证号': 'yonghuIdNumber',
					'医保卡号': 'yonghuKahao',
					'邮箱': 'yonghuEmail',
					'籍贯': 'yonghuJiguan',
					'入院日期': 'yonghuTime',
					//本表字段
					'症状': "zhongyizhenduanText",
					'回复': "zhongyizhenduanHuifuText",
				},
		...
		methods: {
			chartDialog() {
				...
				let params = {
					...
					thisTable: { //当前表
						tableName: "zhongyizhenduan", //当前表表名,
						sumColum: 'zhongyizhenduan_number', //求和字段
						date: 'insert_time', //分组日期字段
						// string : 'zhongyizhenduan_name',//分组字符串字段
						// types : 'zhongyizhenduan_types',//分组下拉框字段
					},
					// joinTable : {//级联表（可以不存在）
				...
				//         groupColumn: "zhongyizhenduan_types",
				//     }
				//     this.$http({
			...
			getDataList() {
				...
				params['zhongyizhenduanDelete'] = 1 // 逻辑删除字段 1 未删除 2 删除


				this.$http({
					url: "zhongyizhenduan/page",
					method: "get",
					params: params
				}).then(({
					data
			...
			// 导入功能上传文件成功后调用导入方法
			zhongyizhenduanUploadSuccess(data) {
				let _this = this;
				_this.$http({
					url: "zhongyizhenduan/batchInsert?fileName=" + data.file,
					method: "get"
				}).then(({
					data
				}) => {
			...
			// 导入功能上传文件失败后调用导入方法
			zhongyizhenduanUploadError(data) {
